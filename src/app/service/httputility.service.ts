...
import { HttpClient, HttpHeaders, HttpResponse, HttpParams, HttpRequest, HttpEventType } from '@angular/common/http';
...
export class HttpUtilityService {
    ...
    //http post method
    post(url:string, data:any, option?:any,background?:boolean): Promise<any> {
    ...
    uploadFileWithProgress(method:string, url:string, data:any, headers?:any) {
        let observable = new Observable<{}>(observer => {
            ...
            try {
              let sub = this.http.request(new HttpRequest(method, environment.serverAddress + "/" + url, data, { reportProgress: true, headers: headers ? headers : this.getHeaders() }))
    ...
    sendRequest(method:string, url:string, data:any, headers?:any) {
        return new Promise((resolve, reject) => {
            try {
                this.helper.startWorkProgressBar();
              this.http.request(new HttpRequest(method, environment.serverAddress + "/" + url, data, { headers: headers ? headers : this.getHeaders() }))
                    .subscribe(res => {
                        resolve(res);
                    }, err => {
                        this.onError(err, this.helper);
